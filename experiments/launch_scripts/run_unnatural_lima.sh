#!/bin/bash
# NOTE: use autoencoding to search the unnatural instruction of shortlima
# Initialization: original perturb
...
target_type=natural_instruction
goal_type=builtin
goal_pos=suffix
...
instruction_init=original_perturb # original, original_perturb, independent_sample, placeholder
...
for ((offset = $start_offset; offset < $end_offset; offset++)); do
  ...
  python -u ../main.py \
    ...
    --config.train_data=vermouthdky/Unnatural_LIMA \
